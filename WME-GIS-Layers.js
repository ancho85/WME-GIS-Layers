...
(async function main() {
  ...
  const SCRIPT_AUTHOR = 'ancho85'; // MapOMatic / JS55CT are the original authors, but they won't fix any Paraguay CartoDB/GeoNode issues
  ...
  function getUrl(extent, gisLayer, zoom) {
    ...
    // ----- ArcGIS -----
    if (gisLayer.platform === 'ArcGIS' || !gisLayer.platform || gisLayer.platform === 'RawPointData') {
	    if (gisLayer.isFeatureSet) {
	        url = gisLayer.url; // no extra filters for this resource (caching)
	        logDebug(`ArcGIS isFeatureSet: Request URL: ${url}`);
	        return url;
	    }
    ...
    if (gisLayer.platform == "GeoNode"){
        ...
        logDebug(`GeoNode: Request URL: ${url}`);
        return url;
    }

    // ----- CartoDB -----
    if (gisLayer.platform == "CartoDB"){
        ...
        url += '&format=GeoJSON';
        logDebug(`CartoDB: Request URL: ${url}`);
        return url;
    }
  ...
  function processLabel(gisLayer, item, displayLabelsAtZoom, area, isPolyLine = false) {
    let fieldValues = [];
    if (["GeoNode", "CartoDB"].indexOf(gisLayer.platform) >= 0){
        fieldValues = item.properties;
    } else if (["RawPointData"].indexOf(gisLayer.platform) >= 0) {
        fieldValues = item;
    } else {
        ...
        fieldValues = item && typeof item === 'object' ? item.attributes || item.properties || item : {};
    }
    let label = '';
  ...
  function processFeaturesGeoNodeCarto(data, token, gisLayer) {
        ...
        onload(res2) {
          if (res2.status < 400) {
            try {
	          ...
	          } else if (["GeoNode", "CartoDB"].indexOf(gisLayer.platform) >= 0){
	            processFeaturesGeoNodeCarto(parsedData, res2.context, gisLayer);
              } else {
  ...
  async function loadSpreadsheetAsync(isoCode, regionCodes) {
    ...
    const SHEET_ID = '1aePOmux2IBxE_2CGPOequGnubr9g4hWr1wH_qAjcM24';
